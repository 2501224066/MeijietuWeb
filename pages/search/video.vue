    ...
    <div>
        ...
        <div class="searchlist a">
            ...
            <div class="selectalign">
                ...
                <a href="javascript:;" class="zhankai" @click="ispalthold">展开 <span></span></a>
            </div>
            <div class="selectalign">
            ...
            <div class="selectalign">
                <div class="selecttitle"> 
                    ...
                    <div class="checkbox" v-show="checkboxshow">
                        <div v-for="(item,index) in mediadata[theme_index].priceclassify" v-show="item.tag==tag" :key="item.priceclassify_id" @click="priceclassify_index=index;checkboxshow=false">{{item.priceclassify_name}}</div>
                    </div>
                </div>
                <div class="selectright">
                    <div>
            ...
            <div class="selectalign">
                ...
                <div class="selectright">
                    <div>
                        ...
                        <div v-if="platform!==''" style="width:auto"><span>平台分类: {{mediadata[theme_index].platform[platform].platform_name}}</span></div>
        ...
        <div class="goodslist a" v-loading="loading">
            ...
            <div class="goodslistcontent goodslistitem" v-for="(item,index) in listdata" :key="index">
                ...
                <div class="cr">
                    <img :src="$store.state.header_img + item.platform_logo">
                </div>
                <div class="cr">
                    <p>总获赞数: {{item.total_like_num==false?'/':item.total_like_num>=100000?parseInt(item.total_like_num/10000)+'W+':item.total_like_num}}</p>  
                </div>
                <div>
...
export default {
   ...
   async asyncData({app}) {
        ...
        let { data:selectgoods } = await selectGoods({modular_id:'3'})
        return { mediadata: data.data[2].theme ,listdata : selectgoods.data.data,
            selectWeixin : selectgoods.data };    
    },
    head(){
        return{
            title:'小红书营销|微信推广|微信KOL|微信公众号 - 媒介兔-全行业&全流程_获客整合营销平台'
        }
    },
    data() {
        return {
    ...
    methods: {
        ...
        qingkong(){
            this.region = this.filed =this.fansnumlevel=this.pricelevel=this.platform=''
        ...
        selectGoods(a){
            ...
            let params = {    
                ...
                included_sataus:'',
                industry_id:'',
                platform_id:this.mediadata[0].platform[this.platform]&&this.mediadata[0].platform[this.platform].platform_id||''
            }
