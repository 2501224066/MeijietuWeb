    ...
    <div class="Orderlist" v-for="(item,index) in indentlist" :key="index">
      ...
      >
        <el-row>
          ...
          <el-col :span="3">
            <p>{{item.status==1?'待接单':item.status==4?'执行中':item.status==7?'审核中':item.status==8?'已完成':item.status==9?'已结算':'已失效'}}</p>
          </el-col>
        </el-row>
      </div>
<!--  ========================================操作按钮============================================= -->
      <div v-if="item.status==1" class="selectbtn">                   
        <p @click="acceptIndent(item.indent_num)">确认接单</p>
        <p class="quxiao" @click="oderCancel(item.indent_num,item.status)">拒绝订单</p>
      </div>
      ...
      <div v-if="item.status==7" class="selectbtn">
        <a @click="upachievements(item.achievements_file)">下载完成信息</a>
        <span v-if="item.achievements_file===''" @click="upload(item.indent_num)">
          <el-upload
            class="upload-demo"
        ...
        <span v-if="item.achievements_file!==''">等待广告主确认</span>
      </div>
    ...
    <div class="noindent" v-show="indentlist.length==0">
      <p class="font14 color666"><i class="el-icon-warning"></i>没有订单，去你挑选需要的媒体吧~</p>
...
export default {
  ...
  methods: {
    ...
    upachievements(a){
       if (a == "") {
          return this.$message({message: '未上传',type: 'warning'})
        }
    ...
    istrueCancal() {
      ...
      this.loading = true;
      if(this.$store.state.userdata.identity!==2){return console.log('用户身份错误'+this.$store.state.userdata.identity)}
      ...
      }else if(this.cancel_status==4){         // =======================交易中媒体主取消订单==================
    ...
    buyerConfirmComplete(a){           //卖家(媒体主)确定订单完成
      if(this.btnwait){return this.$message({message: '操作太频繁,请稍后',type: 'warning'})}
      this.btnwait = true
      this.loading = true
    ...
    this.loading=true
      this.$axios
        ...
        .catch(err => {
          this.$message.error('接单失败:'+err.response.data.message)
    ...
    demand_file(a) {                        //媒体主下载文档
      let href = this.indentdata.data[a].demand_file;
      if (href == "") {
        return this.$message({message: '广告主未上传',type: 'warning'})
      }
